...
class SDE_Breadcrumb_Trail {
	...
	public function __construct( $args = array() ) {
		...
		$defaults = array(
			...
			/* Labels for text used (see SDE_Breadcrumb_Trail::default_labels). */
			'labels' => array()
		);

		$this->args = apply_filters( 'sde_breadcrumb_trail_args', wp_parse_args( $args, $defaults ) );
	...
	public function trail() {
		...
		if ( !empty( $this->items ) && is_array( $this->items ) ) {
			...
			$breadcrumb .= "\n\t\t" . '</' . tag_escape( $this->args['container'] ) . '><div class="deals-clearfix">&nbsp;</div>';
		}

		/* Allow developers to filter the breadcrumb trail HTML. */
		$breadcrumb = apply_filters( 'sde_breadcrumb_trail', $breadcrumb, $this->args );
	...
	public function default_labels() {
		...
		$labels = array(
			'browse'              => __( 'Browse:',                 'wpsdeals' ),
			'home'                => __( 'Home',                    'wpsdeals' ),
			'search'              => __( 'Search results for "%s"', 'wpsdeals' ),
			'error_404'           => __( '404 Not Found',           'wpsdeals' ),
			'paged'               => __( 'Page %d',                 'wpsdeals' ),
			'archives'            => __( 'Archives',                'wpsdeals' ),
			'archive_minute_hour' => __( 'g:i a',                   'wpsdeals' ),
			'archive_minute'      => __( 'Minute %d',               'wpsdeals' ),
			'archive_hour'        => __( 'g a',                     'wpsdeals' ),
			'archive_day'         => __( 'd',                       'wpsdeals' ),
			'archive_week'        => __( 'Week %d',                 'wpsdeals' ),
			'archive_month'       => __( 'F',                       'wpsdeals' ),
			'archive_year'        => __( 'Y',                       'wpsdeals' ),
		//	'edit'                => __( 'Edit',                    'wpsdeals' ), // @todo Implement edit link
		);
	...
	public function do_trail_items() {
		...
		$this->items = apply_filters( 'sde_breadcrumb_trail_items', $this->items, $this->args );
	}
	...
	public function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag )
					$this->items[] = '<a href="' . get_year_link( get_the_time( 'Y', $post_id ) ) . '" title="' . get_the_time( __( 'Y', 'wpsdeals' ), $post_id ) . '">' . get_the_time( $this->args['labels']['archive_year'], $post_id ) . '</a>';

				/* If using the %monthnum% tag, add a link to the monthly archive. */
				elseif ( '%monthnum%' == $tag )
					$this->items[] = '<a href="' . get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'F Y', 'wpsdeals' ), $post_id ) . '">' . get_the_time( $this->args['labels']['archive_month'], $post_id ) . '</a>';

				/* If using the %day% tag, add a link to the daily archive. */
				elseif ( '%day%' == $tag )
					$this->items[] = '<a href="' . get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'F j, Y', 'wpsdeals' ), $post_id ) . '">' . get_the_time( $this->args['labels']['archive_day'], $post_id ) . '</a>';
...
/**
 * Extends the SDE_Breadcrumb_Trail class for bbPress.  Only use this if bbPress is in use.  This should 
 * serve as an example for other plugin developers to build custom breadcrumb items.
 *
 * @since  0.6.0
 * @access public
 */
class bbPress_SDE_Breadcrumb_Trail extends SDE_Breadcrumb_Trail {
	...
	public function do_trail_items() {
		...
		$this->items = apply_filters( 'sde_breadcrumb_trail_get_bbpress_items', $this->items, $this->args );
	}
}

echo sde_breadcrumb_trail( array( 'separator' => '&raquo;', 'before' => '', 'show_browse' => false ) );